...
void productor() {
    for (int i = 1; i <= 30; ++i) {
        ...
        semaforo.notify(); // Notificar al consumidor que hay un nuevo elemento disponible
        std::this_thread::sleep_for(std::chrono::milliseconds(tiempoProductor)); // Espera
    } 
    produccion_terminada = true;
    semaforo.notify(); // Notificar al consumidor que la producción ha terminado
}
...
void consumidor() {
    while (true) {
        ...
        if (produccion_terminada && cola.empty()) {
            break; // Si la producción ha terminado y la cola está vacía, terminar
        }
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(tiempoConsumidor)); // Espera antes de consumir el siguiente elemento
    }
}
